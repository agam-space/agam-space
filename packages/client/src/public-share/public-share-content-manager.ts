import { ContentEntry, FileEntry, FolderEntry } from '../content-tree/entities';
import { ContentTreeV2Store, Sort } from '../content-tree/content-tree.store';
import { decryptEnvelope } from '../crypto/encryption';
import { getPublicShareContentApi } from '../api';
import { decryptFileMetadata } from '../files';
import { isFolderIdRoot, PublicSharedFile } from '@agam-space/shared-types';
import { decryptFolderMetadata } from '../folders/folder-contents';
...
export class PublicShareContentManager {
  ...
  private async getKeyForItem(itemId: string): Promise<Uint8Array> {
    ...
    const itemKey = await decryptEnvelope(fkWrapped, parentKey);
  ...
  async getOrFetch(
    ...
    const encryptedContent = await getPublicShareContentApi(
      this.shareId,
      this.accessToken,
    ...
    if (encryptedContent.itemType === 'file' && encryptedContent.file) {
      ...
      for (const encryptedFolder of encryptedContent.contents.folders) {
        ...
        // Decrypt folder key
        const subFolderKey = await decryptEnvelope(encryptedFolder.fkWrapped, folderKey);
      ...
      for (const encryptedFile of encryptedContent.contents.files) {
        ...
        // Decrypt file key
        const fileKey = await decryptEnvelope(encryptedFile.fkWrapped, folderKey);
